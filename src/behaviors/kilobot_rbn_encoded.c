...
void setRBNparameters(){
  ...
  allocateRBN();
  max_step_value = pow(2, num_nodes/2);
...
void getValues(double *angle, int *step_lenght){
  ...
  *angle = angle_direction/(float)(pow(2, num_values) - 1);

  // Get the step lenght (distance it goes forward)
  int step_value = 0;
  for(int i = num_values; i < num_nodes; i++){
    if(individual_rbn.nodes_state[i] == 1)
      step_value += pow(2, i - num_values);
  }
  if(step_value < (round(max_step_value* 0.80)))
    *step_lenght = rand() % short_step_lenght;
  else if (step_value < (round(max_step_value* 0.95)))
    *step_lenght = (rand() % medium_step_lenght) + short_step_lenght;
  else
    *step_lenght = (rand() % big_step_lenght) + medium_step_lenght;
